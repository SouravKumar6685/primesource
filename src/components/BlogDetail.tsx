...
const BlogDetail: React.FC = () => {
    ...
    const monthString = blog.date_posted ? new Date(blog.date_posted).toLocaleString('default', { month: 'long' }).toUpperCase() : 'MARCH';
    const categoryName = blog.category || 'INSPIRE';

    const renderLeftImages = () => {
        const hasLeft1 = !!blog.left_image_1;
        const hasLeft2 = !!blog.left_image_2;
        const hasLeft3 = !!blog.left_image_3;
        const layout = blog.hero_layout || 'offset-grid';
        ...
        if (!hasLeft1 && !hasLeft2 && !hasLeft3) {
            return (
                <div className="aspect-[4/5] md:aspect-square lg:aspect-[3/4] overflow-hidden bg-gray-100 shadow-xl border border-gray-200">
                    <img
                        src={blog.image_url || 'https://images.unsplash.com/photo-1451187580459-43490279c0fa?q=80&w=2000'}
                        alt={blog.title}
                    ...
                    <div className="relative w-full h-[60vh] md:h-[80vh] lg:h-[70vh] flex items-center justify-center -mt-10">
                        {hasLeft2 && (
                            <div className="w-[80%] shadow-2xl">
                                <img src={blog.left_image_2} alt="Center Large" className="w-full h-auto object-cover rounded-sm" />
                            </div>
                        )}
                    </div>
                );
                    ...
                    <div className="relative w-full h-[60vh] md:h-[80vh] lg:h-[70vh] flex gap-4 items-center justify-center -mt-10">
                        {hasLeft1 && (
                            <div className="w-1/2 h-[80%] shadow-xl">
                                <img src={blog.left_image_1} alt="Left Split" className="w-full h-full object-cover rounded-sm" />
                            </div>
                        )}
                        {hasLeft2 && (
                            <div className="w-1/2 h-[80%] shadow-xl">
                                <img src={blog.left_image_2} alt="Right Split" className="w-full h-full object-cover rounded-sm" />
                            </div>
                        )}
                    </div>
                );
                    ...
                    <div className="relative w-full h-[60vh] md:h-[80vh] lg:h-[70vh] flex gap-4 items-center justify-center -mt-10">
                        {hasLeft1 && (
                            <div className="w-1/2 h-[90%] shadow-xl relative overflow-hidden rounded-sm">
                                <img src={blog.left_image_1} alt="Masonry Tall" className="absolute inset-0 w-full h-full object-cover" />
                            </div>
                        )}
                        <div className="w-1/2 h-[90%] flex flex-col gap-4">
                            {hasLeft2 && (
                                <div className="h-1/2 w-full shadow-lg relative overflow-hidden rounded-sm">
                                    <img src={blog.left_image_2} alt="Masonry Top" className="absolute inset-0 w-full h-full object-cover" />
                                </div>
                            )}
                            {hasLeft3 && (
                                <div className="h-1/2 w-full shadow-lg relative overflow-hidden rounded-sm">
                                    <img src={blog.left_image_3} alt="Masonry Bottom" className="absolute inset-0 w-full h-full object-cover" />
                                </div>
                            )}
                        </div>
                    </div>
            ...
            case 'full-bleed':
                return (
                    <div className="absolute inset-0 w-full h-full z-0">
                        {hasLeft2 && (
                            <img src={blog.left_image_2} alt="Full Bleed" className="w-full h-full object-cover opacity-90" />
                        )}
                    ...
                    <div className="relative w-full h-[60vh] md:h-[80vh] lg:h-[70vh] -mt-10">
                        {hasLeft1 && (
                            <div className="absolute top-[10%] left-[5%] w-[45%] shadow-xl">
                                <img src={blog.left_image_1} alt="Staircase 1" className="w-full h-auto object-cover rounded-sm" />
                            </div>
                        )}
                        {hasLeft2 && (
                            <div className="absolute top-[35%] left-[25%] w-[45%] shadow-2xl z-10">
                                <img src={blog.left_image_2} alt="Staircase 2" className="w-full h-auto object-cover rounded-sm" />
                            </div>
                        )}
                        {hasLeft3 && (
                            <div className="absolute top-[60%] left-[45%] w-[45%] shadow-xl z-20">
                                <img src={blog.left_image_3} alt="Staircase 3" className="w-full h-auto object-cover rounded-sm" />
                            </div>
                        )}
                    </div>
                );
                    ...
                    <div className="relative w-full h-[60vh] md:h-[80vh] lg:h-[70vh] flex items-center justify-center -mt-10">
                        ...
                        {hasLeft2 && (
                            <div className="absolute z-10 w-[60%] lg:w-[65%] shadow-2xl">
                                <img src={blog.left_image_2} alt="Center Large" className="w-full h-auto object-cover" />
                            </div>
                        )}
                        {/* Left Medium Image (Image 1) */}
                        {hasLeft1 && (
                            <div className="absolute z-0 left-0 top-[10%] w-[35%] lg:w-[40%] shadow-xl opacity-90 mix-blend-multiply">
                                <img src={blog.left_image_1} alt="Side Medium 1" className="w-full h-auto object-cover grayscale-[20%]" />
                            </div>
                        )}
                        {/* Right Medium Image (Image 3) */}
                        {hasLeft3 && (
                            <div className="absolute z-20 right-0 bottom-[10%] w-[35%] lg:w-[40%] shadow-2xl border-4 border-[#FAFAFA]">
                                <img src={blog.left_image_3} alt="Side Medium 2" className="w-full h-auto object-cover" />
                            </div>
                        )}
                    </div>
                );
    ...
    return (
        <main className="min-h-screen bg-[#FAFAFA] text-black font-['Inter'] flex flex-col lg:flex-row border-t border-[#FAFAFA]">
            <SEO
                title={blog.title}
                description={blog.content.replace(/[#*`_~>\[\]]/g, '').substring(0, 160) + '...'}
                image={blog.image_url || 'https://images.unsplash.com/photo-1451187580459-43490279c0fa?q=80&w=2000'}
            />
            ...
            <div className="lg:w-[45%] lg:h-screen lg:sticky lg:top-0 lg:border-r border-gray-200 flex">
                <div className="flex-1 p-6 md:p-12 lg:p-16 pt-24 md:pt-32 lg:pt-32 flex flex-col justify-center h-full overflow-hidden bg-white/50 backdrop-blur-sm relative">
                    ...
                    <div className="flex justify-between items-start mb-12 lg:mb-0 lg:absolute lg:top-24 lg:left-16 lg:right-16 z-30">
                        <div className="text-xs font-bold font-mono tracking-widest">{blog.id ? String(blog.id).padStart(2, '0') : '01'}</div>
                        <Link to="/blog" className="lg:hidden font-semibold text-sm hover:opacity-60 transition-opacity">Close</Link>
                    </div>
                    ...
                    <div className="font-['Outfit'] font-bold text-xs tracking-[0.15em] uppercase text-gray-800 mt-8 lg:absolute lg:bottom-12 lg:left-16 z-30">
                        {categoryName} <span className="opacity-50">({monthString})</span>
                    </div>
                </div>
            </div>
            ...
            <div className="lg:w-[55%] min-h-screen px-6 py-8 md:px-16 md:py-16 pt-24 md:pt-32 lg:pt-32 xl:px-24">
                ...
                <article className="max-w-3xl mx-auto xl:mx-0">
                    ...
                    {blog.right_images && blog.right_images.length > 0 && (
                        <div className="mt-24">
                            {blog.right_layout === 'grid' ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
                                    {blog.right_images.map((img, idx) => (
                                ...
                                <div className="flex flex-col space-y-24 md:space-y-36">
                                    {blog.right_images.map((img, idx) => {
                                        ...
                                        if (blog.right_layout === 'stacked-full') {
                                            alignmentClass = "self-center";
                                            widthClass = "w-full";
                                        } else if (blog.right_layout === 'stacked-center') {
                                            alignmentClass = "self-center";
                    ...
                    <div className="mt-32 pt-12 border-t border-gray-200 text-sm font-semibold text-gray-500 uppercase tracking-widest flex justify-between items-center">
                        <div>Written by {blog.author || 'Admin'}</div>
                    </div>
                </article>
            </div>
